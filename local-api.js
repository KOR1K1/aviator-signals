...
export function getFlightResult(bet, currency = "RUB") {
  try {
    ...
    const message = `Вам осталось ${gamesLeft} игр со ставкой ${bet}${currencySymbol}, до заноса X${multiplier} с шансом — ${chance}%`;
    return { message };
  } catch (error) {
...
export function getFlightResult2(bet, currency = "USD") {
  try {
    bet = parseInt(bet, 10) || 0;
    if (bet <= 0) {
    ...
    const message = `You have ${gamesLeft} games left with bet of ${bet}${currencySymbol} before hitting X${multiplier} with a ${chance}% chance`;
    return { message };
  } catch (error) {
