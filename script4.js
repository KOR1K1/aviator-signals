...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function hideFlightAnalysis() {
    ...
    if (flightAnalysisTextElement)
      flightAnalysisTextElement.textContent = "Analysis";
  }
  ...
  async function attemptConnection(serviceName) {
    ...
    for (let i = 0; i <= MAX_RETRIES; i++) {
      if (Math.random() < SUCCESS_CHANCE || i === MAX_RETRIES) {
        ...
        quickLog(
          `${serviceName}: Connection error (attempted ${i + 1})...`,
          "error"
        );
        if (i < MAX_RETRIES) {
  ...
  async function runPreFlightSync() {
    ...
    if (statusLoaderTitle)
      statusLoaderTitle.textContent = "DATA SYNCHRONIZATION";
  ...
  function animateAnalysisTextDots() {
    ...
    if (flightAnalysisTextElement)
      flightAnalysisTextElement.textContent = "Analysis" + dots;
  }
