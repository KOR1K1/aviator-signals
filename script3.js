...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const preloaderImage = preloader
    ? preloader.querySelector(".preloader-image")
    : null;
  ...
  const preloaderHashDisplayElement = document.getElementById(
    "preloaderHashDisplay"
  );
  const siteHashDisplayElement = document.getElementById(
    "siteSessionHashDisplay"
  );
  ...
  const statusLoaderCloseButton = document.getElementById(
    "statusLoaderCloseButton"
  );
  ...
  if (aviaDecorativeImage) {
    ...
    aviaDecorativeImage.style.setProperty(
      "--avia-initial-left",
      aviaInitialLeft
    );
    aviaDecorativeImage.style.setProperty(
      "--avia-initial-opacity",
      aviaInitialOpacity
    );
  }

  const flightAnalysisSection = document.getElementById(
    "flightAnalysisSection"
  );
  const flightAnalysisTextElement =
    document.getElementById("flightAnalysisText");
  const flightProgressBarFillElement = document.getElementById(
    "flightProgressBarFill"
  );
    ...
    setTimeout(() => {
      if (preloader) {
        ...
        if (preloaderImage) {
          ...
          requestAnimationFrame(() =>
            requestAnimationFrame(() => clonedLogo.classList.add("flying"))
          );
        } else {
          if (siteHashDisplayElement)
            siteHashDisplayElement.classList.add("visible");
        ...
        setTimeout(() => {
          ...
          if (
            !preloaderImage &&
            siteHashDisplayElement &&
            !siteHashDisplayElement.classList.contains("visible")
          ) {
            siteHashDisplayElement.classList.add("visible");
          }
  ...
  if (currencyRadioButtons && currencyRadioButtons.length) {
    currencyRadioButtons.forEach((rb) => {
  ...
  function hideFlightAnalysis() {
    ...
    if (flightAnalysisSection) {
      ...
      setTimeout(() => {
        if (
          flightAnalysisSection &&
          !flightAnalysisSection.classList.contains("visible")
        ) {
    ...
    if (flightProgressBarFillElement)
      flightProgressBarFillElement.style.width = "0%";
    if (flightAnalysisTextElement)
      flightAnalysisTextElement.textContent = "Анализ";
  }
  ...
  if (startButtonElement) {
    startButtonElement.addEventListener("click", () => {
      ...
      setTimeout(() => {
        if (
          startButtonElement &&
          !startButtonElement.classList.contains("visible")
        )
      ...
      if (backButtonElement) {
        ...
        setTimeout(() => {
          if (
            backButtonElement &&
            !backButtonElement.classList.contains("visible")
          )
      ...
      if (ballContainerElement)
        ballContainerElement.classList.remove("visible");
      if (statusLoaderOverlay) {
        statusLoaderOverlay.classList.remove("visible");
        setTimeout(() => {
          if (
            statusLoaderOverlay &&
            !statusLoaderOverlay.classList.contains("visible")
          )
  ...
  function updateConfirmButtonState() {
    ...
    if (isValid) {
      ...
      confirmButton.disabled = true;
      idErrorMessage.textContent = genericErrorMessage;
  ...
  async function attemptConnection(serviceName) {
    ...
    for (let i = 0; i <= MAX_RETRIES; i++) {
      if (Math.random() < SUCCESS_CHANCE || i === MAX_RETRIES) {
        ...
        quickLog(
          `${serviceName}: Ошибка подключения (попытка ${i + 1})...`,
          "error"
        );
        if (i < MAX_RETRIES) {
  ...
  async function runPreFlightSync() {
    ...
    if (statusLoaderTitle)
      statusLoaderTitle.textContent = "СИНХРОНИЗАЦИЯ ДАННЫХ";
  ...
  async function runPostFlightStatus() {
    ...
    analysisDisplay.textContent = "Итог";
    await new Promise((resolve) => setTimeout(resolve, 1500));

    async function fetchFlightResult(bet, currency) {
      return getFlightResult(bet, currency).message;
    }
    const finalResultMessageText = await fetchFlightResult(
      accountId || "0",
      selectedCurrency
    );
    // Разделяем по запятой
    const [first, second] = finalResultMessageText.split(",");
    if (first && second) {
      // Делаем первую букву второй части заглавной
      const secondLine =
        second.trim().charAt(0).toUpperCase() + second.trim().slice(1);
      statusLoaderMessage.innerHTML = `${first}<br>${secondLine}`;
    } else {
  ...
  function animateAnalysisTextDots() {
    ...
    if (flightAnalysisTextElement)
      flightAnalysisTextElement.textContent = "Анализ" + dots;
  }

  function startGameAnimation() {
    if (
      ballContainerElement &&
      !ballContainerElement.classList.contains("visible")
    ) {
      ballContainerElement.style.opacity = "1";
      ballContainerElement.classList.add("visible");
    }
    ...
    if (flightAnalysisSection) {
      ...
      requestAnimationFrame(() =>
        flightAnalysisSection.classList.add("visible")
      );
    }
    if (flightProgressBarFillElement)
      flightProgressBarFillElement.style.width = "0%";
    ...
    function animateFrame(currentTime) {
      ...
      if (elapsedTime < duration) {
        ...
        const rotation =
          Math.sin(elapsedTime * frequencyRotate) * amplitudeRotate;
        ...
        if (ballImage)
          ballImage.style.transform = `translateY(${finalY}px) rotate(${rotation}deg)`;
        ...
        if (ballImage)
          ballImage.style.transform = "translateY(0px) rotate(0deg)";
        if (ballContainerElement)
          ballContainerElement.classList.remove("visible");

        if (flightProgressBarFillElement)
          flightProgressBarFillElement.style.width = "100%";
        if (analysisDotsIntervalId) {
          clearInterval(analysisDotsIntervalId);
          analysisDotsIntervalId = null;
        }
      ...
      if (statusLoaderOverlay) {
        ...
        setTimeout(() => {
          if (
            ...
            statusLoaderOverlay.style.display = "none";
            if (statusLoaderCloseButton)
              statusLoaderCloseButton.style.display = "none";
          }
        }, 200);
      }
  ...
  if (languageDropdownBtn && languageDropdownList) {
    languageDropdownBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      languageDropdownList.classList.toggle("show");
    });
    document.addEventListener("click", (e) => {
      if (
        !languageDropdownList.contains(e.target) &&
        e.target !== languageDropdownBtn
      ) {
        languageDropdownList.classList.remove("show");
      }
    });
  }
});
